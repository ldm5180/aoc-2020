...
constexpr std::string_view input = R"(17-19 p: pwpzpfbrcpppjppbmppp
...
constexpr auto line_to_password(std::string_view line) {
  Password p{};
  auto start = 0u;
...
constexpr auto lines_to_passwords(const auto lines) {
  std::array<Password, std::size(lines)> r{};

  for (auto i = 0u; i < std::size(lines); ++i) {
    r[i] = line_to_password(lines[i]);
  }
  return r;
}
...
constexpr auto valid_count(const auto &passwords) {
  return std::count_if(std::begin(passwords), std::end(passwords),
                       [](const auto password) { return is_valid(password); });
}

constexpr int answer() {
  constexpr auto num_lines = line_count(input);
  constexpr auto lines = lines_array<num_lines>(input);
  constexpr auto passwords = lines_to_passwords(lines);
  constexpr auto result = valid_count(passwords);
  return result;
}

#include <iostream>
int main() {
  constexpr auto r = answer();
  std::cout << "02 ---> " << r << std::endl;
  return r;
}
