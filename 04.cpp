...
template <auto num_passports> constexpr auto passports_array(std::string_view input) {
  ...
  for (auto i = 0u; i < num_passports && end != std::string_view::npos; ++i) {
    end = input.find("\n\n", start);
    passports[i] = input.substr(start, end - start);
    start = end + 2;
  }
...
#include <iostream>
int main() {
  constexpr auto num_passports = passport_count(input);
  constexpr auto passports = passports_array<num_passports>(input);
  constexpr auto answer = num_valid(passports);
  std::cout << "04 ---> " << answer << std::endl;
}
