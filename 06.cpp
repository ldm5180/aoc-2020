...
constexpr std::string_view input = R"(jmcvr
marvj
...
constexpr auto group_count(std::string_view input) {
  auto count = 0;
...
template <auto num_groups>
constexpr auto groups_array(std::string_view input) {
  std::array<std::string_view, num_groups> groups{};
  std::size_t start{};
  std::size_t end{};

  for (auto i = 0u; i < num_groups && end != std::string_view::npos; ++i) {
    end = input.find("\n\n", start);
    groups[i] = input.substr(start, end - start);
    start = end + 2;
  }
...
constexpr auto sum_groups(const auto groups) {
  return std::accumulate(std::begin(groups), std::end(groups), 0,
                         [](const auto accum, const auto group) {
                           return accum + num_unique(group);
                         });
}

#include <iostream>
int main() {
  constexpr auto num_groups = group_count(input);
  constexpr auto groups = groups_array<num_groups>(input);
  constexpr auto answer = sum_groups(groups);
  std::cout << "06 ---> " << answer << std::endl;
  return answer;
}
